...
命令:
  ...
  repair-schema     补全数据库缺列并重启 backend（修复接口 500）
...
fix_script_permissions() {
  local f
  for f in "$0" update.sh deploy.sh install-server.sh docker/deploy.sh scripts/mysql-daily-backup.sh; do
    [[ -f "$ROOT/$f" ]] || continue
...
ensure_env_file() {
  ...
  if ! grep -q '^MYSQL_PUBLISH_HOST=' "$ENV_FILE" 2>/dev/null; then
    set_env_value "MYSQL_PUBLISH_HOST" "0.0.0.0"
    ok "已设置 MYSQL_PUBLISH_HOST=0.0.0.0（允许远程直连 MySQL，请在安全组放行 MYSQL_PORT）"
  fi
...
load_backup_config() {
  if [[ -f "$ENV_FILE" ]]; then
    ...
    BACKUP_CRON_HOUR="$(read_env_value BACKUP_CRON_HOUR "$BACKUP_CRON_HOUR")"
    BACKUP_CRON_MINUTE="$(read_env_value BACKUP_CRON_MINUTE "$BACKUP_CRON_MINUTE")"
  fi
  mkdir -p "$BACKUP_DIR"
}
...
install_backup_cron() {
  ...
  local backup_root backup_log runner cron_env
  backup_root="$(dirname "$backup_dir")"
  backup_log="${backup_root}/backup.log"
  runner="$ROOT/scripts/mysql-daily-backup.sh"
  cron_env="ENV_FILE=${ENV_FILE} BACKUP_DIR=${backup_dir} BACKUP_KEEP_DAYS=${keep_days}"
  ...
  if [[ "$(id -u)" -ne 0 ]]; then
    ...
    local cron_line="${cron_minute} ${cron_hour} * * * cd ${ROOT} && ${cron_env} ${runner} >> ${backup_log} 2>&1"
...
${cron_minute} ${cron_hour} * * * root cd ${ROOT} && ${cron_env} ${runner} >> ${backup_log} 2>&1
EOF
  ...
  if [[ "$trial" == "1" ]]; then
    info "立即试跑一次: ${runner}"
    ${cron_env} "$runner" || warn "试跑失败，请检查 MySQL 容器是否运行"
  fi
}

ensure_backup_cron() {
  [[ "${SKIP_BACKUP_CRON:-}" == "1" ]] && return 0
  info "检查 MySQL 每日自动备份任务..."
  local runner="$ROOT/scripts/mysql-daily-backup.sh"
  if [[ "$(id -u)" -ne 0 ]] && [[ -f "$BACKUP_CRON_FILE" ]]; then
    if grep -q 'mysql-daily-backup.sh' "$BACKUP_CRON_FILE" 2>/dev/null \
      && ! grep -q "$runner" "$BACKUP_CRON_FILE" 2>/dev/null; then
      warn "系统 cron 路径与当前项目目录不一致，请执行: sudo $0 backup-setup-cron"
    fi
  fi
  install_backup_cron 0 || warn "自动备份 cron 安装失败，请手动: $0 backup-setup-cron"
}
...
cmd_repair_schema() {
  ...
  info "补全数据库缺列（apifox / 软删除 / version 等）..."
  local script="/app/docker/pre_bootstrap_columns.py"
  if docker exec ai-platform-backend test -f "$script" 2>/dev/null; then
    docker exec ai-platform-backend python "$script" || warn "部分列补全失败，见上方日志"
  else
    warn "当前 backend 镜像无 pre_bootstrap_columns.py，将重新构建部署..."
    compose_cmd up -d --build --force-recreate backend
    wait_backend_ready || error "backend 未就绪，请: ./linux-deploy.sh logs backend"
    ok "已通过重建 backend 完成补列"
    return 0
  fi

  info "重启 backend ..."
  compose_cmd restart backend
  wait_backend_ready || warn "backend 未就绪，请: ./linux-deploy.sh logs backend"
  ok "数据库结构修复完成，请刷新浏览器验证接口目录"
}
...
cmd_diagnose() {
  fix_script_permissions
  ensure_docker
  local http_port backend_port mysql_port
  http_port="$(read_env_value HTTP_PORT 5173)"
  backend_port="$(read_env_value BACKEND_PORT 8000)"
  mysql_port="$(read_env_value MYSQL_PORT 3245)"

  echo "========== 容器状态 =========="
  compose_cmd ps || true
  echo
  echo "========== 端口监听 =========="
  ss -tlnp 2>/dev/null | grep -E ":${http_port}|:${backend_port}|:${mysql_port}" || netstat -tlnp 2>/dev/null | grep -E ":${http_port}|:${backend_port}|:${mysql_port}" || true
  echo
  echo "========== 后端直连 =========="
...
cmd_up() {
  ...
  info "停止旧 backend，释放 MySQL 表锁..."
  compose_cmd stop backend 2>/dev/null || true
  docker rm -f ai-platform-backend 2>/dev/null || true
...
main() {
  ...
  repair-schema|fix-schema) cmd_repair_schema ;;
