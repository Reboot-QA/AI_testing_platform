...
> 详细规范（不复制、只链接）：[backend/docs/后端开发规范.md](../../backend/docs/后端开发规范.md) —— 分层、目录、命名、Pydantic、DB、Repository、异常、并发、日志、测试、提交。
...
- 接口自动化只剩新模块 `apifox`（分层重构，前后端 `apifox/` 子目录，API 前缀 `/api/v1/apifox/*`）。旧 `api_automation` 已下线移除（含遗留 7 表，2026-07 清除）。注意 `services/api_automation_migration.py` 名不副实（实为通用迁移，保留）、前端 `api_automation_mgmt` 是 apifox 菜单命名（保留）。
