...
> Spec 驱动工作流的横向技能：`grill-with-docs` → `to-spec` → `implement-and-ship`，其中任一端做完就停时走本技能。规则见 `.Codex/rules/spec-workflow.md`。
...
- **不要**用 `glab issue note -m "中文"`：本机 GBK 控制台会把中文双重编码写坏（Issue #15、#21 前车之鉴），且 glab 1.108.0 无 `--message-file`。脚本内部走 `glab api --input` 并自动读回校验，见 `.Codex/rules/spec-workflow.md`「中文编码硬约束」。
...
### 1. 重建上下文（按序读，不要跳）
```bash
python .Codex/scripts/glab_cn.py show <ID> --out <scratchpad>/issue<ID>.md
```
