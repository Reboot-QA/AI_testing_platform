...
> Spec 驱动工作流第 2 步：`grill-with-docs` → 本技能 → `implement-and-ship`（跨端接力走 `handoff`）。规则见 `.Codex/rules/spec-workflow.md`。
...
```bash
python .Codex/scripts/glab_cn.py create --body-file <scratchpad草稿路径>
```
...
- 非 0 → **不要继续**，排查后用 `python .Codex/scripts/glab_cn.py update <ID> --body-file <草稿>` 重发。
