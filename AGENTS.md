...
本仓库 AI 上下文按「五层知识体系」组织，Codex 与 Cursor 共享同一份规则（单一事实来源）：

| 层 | 位置 | 作用 |
| --- | --- | --- |
| 1 个人全局 | `~/.Codex/AGENTS.md`（仓库外） | 个人偏好，跨项目生效；Cursor 对应 Settings → User Rules |
| 2 项目共享 | **本文** | 概述 / 命令 / 架构，常驻加载 |
| 3 项目私有 | `Codex.local.md`（已 gitignore） | 本地 proxy target / 端口 / 当前调试任务 |
| 4 领域规则 | `.Codex/rules/*.md`（唯一正文）+ `.cursor/rules/*.mdc`（Cursor 薄封装） | 前端/后端/Git 硬约束；子目录 `AGENTS.md` 按路径 `@import` |
| 5 技能 | `~/.Codex/skills/`（个人通用）+ `.Codex/skills/`（项目专属） | 可复用工作流 |

- **规则单一事实来源在 `.Codex/rules/`**：`frontend.md`、`backend.md`、`git-workflow.md`、`spec-workflow.md`。改规则改这四份；`frontend/AGENTS.md` / `backend/AGENTS.md` 与 `.cursor/rules/*.mdc` 都是引用它们的薄封装。
- **Spec 驱动 + Issue/MR 半自主工作流**：见 `.Codex/rules/spec-workflow.md`（配套技能 `.Codex/skills/{grill-with-docs,to-spec,implement-and-ship,handoff}`）。对齐 → 出规格发 Issue → 实施建 MR，纯 `glab`、合 `dev`、不自动关 Issue。**Spec 正文的单一事实来源是 GitLab Issue**，仓库内不存 Spec 文件；跨端/跨会话接力的交接物是 Issue 评论（`handoff` 技能）。中文 Issue 正文**必须**走 `.Codex/scripts/glab_cn.py`（本机 GBK 控制台 + glab 无 `--description-file`，拼参数会写坏正文）。
- 详细开发规范（不复制、仅链接）：前端见 `frontend/docs/*.md`，后端见 `backend/docs/后端开发规范.md`。
...
- Vite 将 `/api/` 代理到 **`vite.config.ts`** 中的 target（当前指向演示服务器 `43.160.226.39`）；本地联调后端时需改为 `http://127.0.0.1:8000`，但**不要把这个改动提交**（详见 `Codex.local.md`）。
...
旧 `api_automation` 模块**已下线并移除**（路由/服务/前端 `ApiAutomation.vue` 先期删除，遗留模型与 7 张表于 2026-07 清除）。现仅保留新模块：

- **`apifox`**（Apifox 式分层重构，当前开发重心）：前后端均放在 `apifox/` 子目录，路由 `/apifox`（工作台）与 `/apifox/project/:projectId`（项目工作区），API 前缀 `/api/v1/apifox/*`。
- ⚠️ 命名残留（**不是旧模块**，勿删）：前端菜单组 `api_automation_mgmt` / 权限 `api_automation` 是 apifox 的菜单命名；`services/api_automation_migration.py` 名不副实，实为通用迁移（created_by / departments 回填）。
...
- 后端硬约束详见 `.Codex/rules/backend.md`（→ `backend/docs/后端开发规范.md`）。
...
- 前端硬约束（TS/SFC/API 类型/token/300 行等）详见 `.Codex/rules/frontend.md`（→ `frontend/docs/*.md`）。改前端代码前必读。
