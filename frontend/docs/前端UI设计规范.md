...
6. **标题责任按 Shell 区分**：Hub/项目工作区由页面或面板展示，系统页由业务页展示；同一视图不重复标题。
...
- 正文：`--ax-text-body-size` / `--ax-text-body-line`（15px / 1.5）。
- 密集表格与专业编辑器：`--ax-text-body-sm-size`（14px）。
- 说明与元信息：`--ax-text-caption-size` / `--ax-text-caption-line`（13px / 1.35）。
- 标题：`--ax-text-title-sm-size`、`--ax-text-title-size`、`--ax-text-heading-size`、`--ax-text-display-size`。
- 行高：`--ax-leading-tight/compact/normal/relaxed/prose`。
- 页面、卡片、工具栏：`--ax-page-padding-*`、`--ax-card-padding`、`--ax-toolbar-*`。
- 原子间距遵循 4px 基础网格（必要时允许 2px 半格），使用 `--ax-space-*`；旧 `--ax-gap-*` 仅作兼容别名。

普通搜索、筛选与主操作使用 Element Plus 默认尺寸；`small` 仅用于表格行内操作、徽标和高密度编辑器。正文不得低于 13px，10–12px 只允许用于徽标或极短元信息；代码编辑器和图标尺寸按组件语义单独处理。
...
约定：
- 标题按壳层负责：Hub 首页可使用页面级标题；项目工作区以当前面板标题为主；系统管理业务页显示页面标题。任一页面只保留一个主标题。
...
> `.table-fill` / `.pagination-bar` / `.page-col` 已是**全局工具类**（`src/styles/layout.css`，`main.ts` 引入），页面直接写类名即可，**不用再在 scoped 里定义**。
...
| 当前一级/二级导航 | `src/components/shell/GlobalRail.vue`、`WorkspaceTree.vue`、`src/layouts/SystemShell.vue` |
