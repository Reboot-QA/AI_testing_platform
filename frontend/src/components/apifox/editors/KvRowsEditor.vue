  ...
  <div class="kv-editor">
    ...
    <template v-else>
      <div v-for="(row, i) in rows" :key="i" class="kv-row" :class="{ 'kv-row-off': !row.enabled }">
        ...
        />
        <VarInput v-else v-model="row.key" size="small" placeholder="键" class="kv-key" />

        <VarInput v-model="row.value" size="small" placeholder="值" class="kv-val" />
      ...
      <!-- 自动生成的默认头：默认隐藏、可展开；只读展示（计算类显示「发送时计算」，固定值真实发送） -->
      <div v-if="suggest === 'header' && visibleAutoHeaders.length" class="auto-headers">
        <el-button link size="small" @click="showAuto = !showAuto">
          <el-icon><component :is="showAuto ? 'View' : 'Hide'" /></el-icon>
          {{ showAuto ? '隐藏自动生成的' : `显示自动生成的（${visibleAutoHeaders.length}）` }}
        </el-button>
        <div v-if="showAuto" class="auto-list">
          <div v-for="a in visibleAutoHeaders" :key="a.name" class="auto-row">
            <el-icon class="auto-check"><CircleCheck /></el-icon>
            <span class="auto-name">{{ a.name }}</span>
            <el-tooltip :content="a.description" placement="top">
              <el-icon class="auto-info"><InfoFilled /></el-icon>
            </el-tooltip>
            <span class="auto-val" :class="{ 'auto-val-computed': a.computed }">{{ a.value }}</span>
            <span class="auto-type">string</span>
          </div>
        </div>
      </div>
...
import { LONG_TEXT_MAX_LEN } from '@/constants/limits'
...
// 自动生成的默认头（后端为单一事实来源）：默认隐藏、可展开；用户已配置同名的不展示
const autoHeaders = ref<Schemas['DefaultHeaderOut'][]>([])
...
onMounted(() => {
  syncTail()
  if (props.suggest === 'header') {
    apifoxApi.listDefaultHeaders().then((r) => (autoHeaders.value = r))
  }
})
