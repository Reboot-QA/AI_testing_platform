...
/** 当前选中分组（null=未分组）；与接口树 selectedFolderId 同语义，新建场景挂到此分组 */
...
// 文件夹被删后，若当前选中已不存在则回落到未分组
watch(
  () => props.folders,
  (list) => {
    const id = selectedFolderId.value
    if (id != null && !list.some((f) => f.id === id)) selectedFolderId.value = null
  },
)
...
.group-head--folder:hover,
.group-head--ungrouped:hover {
...
.empty-hint {
  pointer-events: none;
  padding: var(--ax-space-0-5) 0 var(--ax-space-0-5) var(--ax-space-7);
  font-size: var(--ax-font-xs);
  line-height: var(--ax-leading-compact);
  color: var(--ax-text-placeholder);
}
