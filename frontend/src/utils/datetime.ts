...
/** 解析 API 无时区 datetime（手工测试单 started/finished/executed 等，按北京时间 wall clock 写入） */
export function parseShanghaiNaiveDateTime(
  ...
  const text = String(value).trim()
  if (!text) return null
  const iso = (text.includes('T') ? text : text.replace(' ', 'T')).slice(0, 19)
...
/** 无时区字段已为北京时间，勿按 UTC 解析（与 formatBeijingTime / created_at 区分） */
export function formatBeijingWallClock(
  value: string | Date | number | null | undefined,
  fallback = '-',
): string {
  const date = parseShanghaiNaiveDateTime(value)
  if (!date) return fallback
  return date.toLocaleString('zh-CN', {
    timeZone: BEIJING_TZ,
    hour12: false,
