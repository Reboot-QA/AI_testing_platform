...
export function processorsToLegacy(pre: Processor[], post: Processor[]) {
  ...
  // 内联脚本（script_inline）无法用 script_id 表达，单独派生给 debug 直发，避免被静默丢弃
  const inline = (arr: Processor[]) =>
    arr
      .filter((p) => p.kind === 'script_inline')
      .map((p) => ({
        content: p.content ?? '',
        lang: p.script_lang || 'javascript',
        enabled: p.enabled,
      }))
  // 等待（wait）项：debug 旧管线无 wait 概念，单独派生毫秒列表给直发，避免「等待」被静默丢弃
  const waits = (arr: Processor[]) =>
    arr.filter((p) => p.kind === 'wait' && p.enabled !== false).map((p) => p.wait_ms ?? 0)
  const contract = post.find((p) => p.kind === 'contract')
  return {
    pre_scripts: scripts(pre),
    post_scripts: scripts(post),
    pre_inline: inline(pre),
    post_inline: inline(post),
    pre_waits: waits(pre),
    post_waits: waits(post),
    assertions: post
