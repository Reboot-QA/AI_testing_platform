      ...
      <el-table v-loading="loading" :data="users" stripe border height="100%">
        ...
        <el-table-column prop="is_active" label="状态" width="100">
          <template #default="{ row }">
            <el-tag v-if="row.login_locked" type="danger" size="small">已锁定</el-tag>
            <el-tag v-else-if="!row.is_active" type="info" size="small">禁用</el-tag>
        ...
        <el-table-column label="操作" width="340" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              v-if="row.id !== currentUserId && !row.login_locked"
              link
              type="danger"
              @click="handleLockLogin(row)"
            >
              锁定
            </el-button>
            <el-button v-if="row.login_locked" link type="success" @click="handleUnlockLogin(row)">
              解锁
            </el-button>
            <el-button
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑用户' : '添加用户'" width="520px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="姓名">
          <el-input v-model="form.full_name" :maxlength="TITLE_MAX_LEN" />
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <el-input v-model="form.email" :maxlength="TITLE_MAX_LEN" placeholder="选填" />
        </el-form-item>
    ...
    <el-dialog v-model="passwordDialogVisible" title="重置密码" width="420px">
      ...
      >
        <el-form-item label="用户">
          <el-input :maxlength="SECRET_MAX_LEN" :model-value="passwordTarget?.username" disabled />
        </el-form-item>
...
import { SECRET_MAX_LEN, TITLE_MAX_LEN } from '@/constants/limits'
...
async function handleLockLogin(row: User) {
  await ElMessageBox.confirm(`确认锁定用户「${row.username}」的登录？`, '提示', { type: 'warning' })
  await userApi.lockLogin(row.id)
  ElMessage.success('账号已锁定')
  loadData()
}

async function handleUnlockLogin(row: User) {
  await userApi.unlockLogin(row.id)
