      ...
      <div class="panel config-panel">
        ...
        <div class="panel-body">
          <el-form ref="formRef" :model="form" :rules="rules" label-width="88px" class="gen-form">
            <el-form-item v-if="!scoped" label="项目" prop="project_id">
              <el-select
...
const form = reactive<GenerateForm>({
  project_id: props.scopedProjectId ?? null,
  provider_id: null,
  requirement_ids: [],
  requirement_text: '',
...
const selectedProvider = computed(
  () => llmProviders.value.find((item) => item.id === form.provider_id) ?? null,
)
...
async function retryFailed() {
  const ids = failedRequirements.value
    .map((f) => f.requirement_id)
    .filter((id): id is number => id != null)
  if (!ids.length) {
    ElMessage.info('失败项无关联需求，请手动调整后重新生成')
    return
  }
  await formRef.value?.validate()
  form.requirement_ids = ids
  fillFromRequirement()
  const generateCount = resolveGenerateCount(ids)
  await aiStore.startGeneration(
    {
      project_id: form.project_id!,
      provider_id: form.provider_id!,
...
function goToTestCaseLibrary() {
  const projectId = props.scopedProjectId ?? form.project_id
  if (!projectId) {
...
onMounted(async () => {
  registerAssistantHandler('aiGenerate.prepareDemo', async () => {
    ...
    // 新壳工作区内锁定当前项目；独立页时回退首个项目
    form.project_id = props.scopedProjectId ?? projects.value[0].id
...
.failed-panel {
  ...
  border: 1px solid color-mix(in srgb, var(--el-color-warning) 40%, white);
  background: color-mix(in srgb, var(--el-color-warning) 8%, white);
}
...
.failed-list li {
  ...
  padding: var(--ax-space-1-5) 0;
  border-top: 1px solid color-mix(in srgb, var(--el-color-warning) 20%, white);
  font-size: var(--ax-text-caption-size);
}
