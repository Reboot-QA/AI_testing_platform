<template>
  <PageCard fill>
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑用例' : '添加用例'" width="640px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="测试步骤">
          <el-input v-model="form.steps" :maxlength="LONG_TEXT_MAX_LEN" type="textarea" :rows="4" />
        </el-form-item>
    ...
    <TestCaseDetailDrawer v-model="drawerVisible" :testcase="detail" @saved="handleDetailSaved" />
    ...
    <el-dialog v-model="importDialogVisible" title="导入用例" width="520px">
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        title="覆盖：清空当前项目全部用例（手工测试单关联行同步清理），以 Excel 为准重新导入。"
        style="margin-bottom: var(--ax-space-4)"
      />
      <el-upload
        ref="uploadRef"
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">
            Excel 需包含「标题」列；文件内同标题多行会合并为一条（保留最后一行）；XMind 导入末级节点
          </div>
        </template>
      </el-upload>
      <template #footer>
...
// scopedProjectId 传入时锁定该项目（新壳工作区）：隐藏项目下拉、关闭「全部项目」；不传保持独立页原行为
...
function handleDetailSaved(updated: TestCase) {
...
async function handleImport() {
  ...
  if (importMode.value === 'replace') {
    try {
      await ElMessageBox.confirm(
        '覆盖导入将清空当前项目全部用例（手工测试单中的关联行也会清理），并以 Excel 为准重新导入。是否继续？',
        '确认覆盖导入',
  ...
  try {
    const res = await testcaseApi.importFile(projectId.value, importFile.value, importMode.value)
    if (res.imported_count === 0) {
      ElMessage.warning(res.message || '未导入任何用例')
    } else {
...
function applyListFilterFromHash() {
  ...
  else if (filter === 'review_pending') filterStatus.value = 'pending'
  else if (filter === 'ai_generated') filterSource.value = 'ai_generated'
  else if (filter === 'ai_pending') {
    filterSource.value = 'ai_generated'
