...
/** 是否为 HTTP 401（网络/5xx 等瞬时失败勿据此清会话） */
...
request.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      const url = error.config?.url || ''
      // /auth/me 交给路由守卫软跳转，避免与 next('/login') 叠一次硬刷新
      const letRouterHandle = url.includes('/auth/me')
      if (!letRouterHandle && !window.location.pathname.includes('/login')) {
