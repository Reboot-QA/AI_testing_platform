...
import { CatalogType, ContentType, MenuItemType } from '@/apifox2/enums'
import type { UnsafeAny } from '@/apifox2/types'
...
export function isMenuSameGroup(left: ApiMenuData, right: ApiMenuData): boolean {
  const httpGroup = [MenuItemType.ApiDetailFolder, MenuItemType.ApiDetail, MenuItemType.Doc]

  const schemaGroup = [MenuItemType.ApiSchemaFolder, MenuItemType.ApiSchema]

  const requestGroup = [MenuItemType.RequestFolder, MenuItemType.HttpRequest]
