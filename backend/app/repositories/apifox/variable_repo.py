...
def delete_env_locals(db: Session, var_id: int) -> None:
    """删变量前先清其各用户本地值，避免外键约束报错（MySQL）。"""
    db.query(ApifoxEnvironmentVarLocal).filter(
        ApifoxEnvironmentVarLocal.environment_variable_id == var_id
    ).delete(synchronize_session=False)


def delete_global_locals(db: Session, var_id: int) -> None:
    db.query(ApifoxGlobalVarLocal).filter(
