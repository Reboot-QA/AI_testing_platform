...
def list_scenarios(db: Session, project_id: int) -> List[ApifoxScenario]:
    return (
        ...
        .filter(ApifoxScenario.project_id == project_id, ApifoxScenario.deleted_at.is_(None))
        .order_by(ApifoxScenario.sort_order, ApifoxScenario.id)
        .all()
    )


def list_deleted_scenarios(db: Session, project_id: int) -> List[ApifoxScenario]:
    """回收站：项目内已软删除的场景（最近删的在前）。"""
    return (
        db.query(ApifoxScenario)
        .filter(ApifoxScenario.project_id == project_id, ApifoxScenario.deleted_at.is_not(None))
        .order_by(ApifoxScenario.deleted_at.desc(), ApifoxScenario.id.desc())
        .all()
    )
...
def detach_case_refs(db: Session, case_ids: set[int]) -> int:
    """移除场景步骤中对给定用例的引用（不 commit）。"""
    if not case_ids:
        return 0
    return (
        db.query(ApifoxScenarioStep)
        .filter(ApifoxScenarioStep.ref_case_id.in_(case_ids))
