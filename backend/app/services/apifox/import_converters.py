...
def to_openapi3(raw: str) -> Dict[str, Any]:
    ...
    if isinstance(doc, dict):
        ...
        if doc.get("apifoxProject") is not None or "apiCollection" in doc:
            return apifox_to_openapi3(doc)
        if isinstance(doc.get("info"), dict) and "item" in doc:
            return postman_to_openapi3(doc)
        raise ValueError("无法识别的 JSON（需 OpenAPI 3.x / Swagger 2.0 / Apifox / Postman Collection）")

    if "<jmeterTestPlan" in text:
        return jmeter_to_openapi3(text)

    if text.lower().startswith("curl"):
        return curl_to_openapi3(text)

    raise ValueError("无法识别的导入内容（支持 OpenAPI/Swagger/Apifox JSON、Postman Collection、JMeter .jmx、cURL）")
...
# ---------- Apifox 导出格式 ----------
_APIFOX_PARAM_IN = ("query", "path", "header", "cookie")


def _apifox_params(parameters: Any) -> List[Dict[str, Any]]:
    """Apifox api.parameters（按位置分组的 dict）→ OpenAPI parameters 列表。"""
    rows: List[Dict[str, Any]] = []
    if not isinstance(parameters, dict):
        return rows
    for loc in _APIFOX_PARAM_IN:
        for p in parameters.get(loc) or []:
            ...
            if p.get("example") is not None:
                row["example"] = p["example"]
            rows.append(row)
    return rows


def apifox_to_openapi3(doc: Dict[str, Any]) -> Dict[str, Any]:
    paths: Dict[str, Any] = {}
    ...
    def walk(node: Any, folder: Optional[str]) -> None:
        ...
        if isinstance(api, dict) and api.get("path"):
            ...
            if method not in _HTTP_METHODS:
                return
            op: Dict[str, Any] = {
                "summary": api.get("name") or node.get("name") or api["path"],
                "tags": [folder] if folder else [],
                "parameters": _apifox_params(api.get("parameters")),
                "responses": {},
            }
            body = api.get("requestBody")
            if isinstance(body, dict) and isinstance(body.get("jsonSchema"), dict):
                op["requestBody"] = {"content": {"application/json": {"schema": body["jsonSchema"]}}}
            paths.setdefault(api["path"], {})[method] = op
            return
        # 目录节点：以本级 name 作为下级 tag 继续下钻
        walk(node.get("items"), node.get("name") or folder)

    walk(doc.get("apiCollection"), None)
    if not paths:
        raise ValueError("Apifox 数据中未找到可导入的接口")
    return {
        "openapi": "3.0.0",
        "info": doc.get("info") if isinstance(doc.get("info"), dict) else {"title": "Apifox 导入"},
        "paths": paths,
    }


# ---------- JMeter .jmx ----------
def _jmx_string_props(sampler: ET.Element) -> Dict[str, str]:
    return {
        p.get("name", ""): (p.text or "")
        for p in sampler.findall("stringProp")
        if p.get("name")
    }


def _jmx_bool_prop(sampler: ET.Element, name: str) -> bool:
    el = sampler.find(f"boolProp[@name='{name}']")
    return el is not None and (el.text or "").strip().lower() == "true"


def _jmx_arguments(sampler: ET.Element) -> List[Tuple[str, str]]:
    """取 HTTPSampler 的参数（name, value）；raw body 时 name 为空。"""
    args: List[Tuple[str, str]] = []
    for el in sampler.iterfind(".//elementProp[@elementType='HTTPArgument']"):
        name = value = ""
        for sp in el.findall("stringProp"):
            if sp.get("name") == "Argument.name":
                name = sp.text or ""
            elif sp.get("name") == "Argument.value":
                value = sp.text or ""
        args.append((name, value))
    return args


def jmeter_to_openapi3(text: str) -> Dict[str, Any]:
    try:
        root = ET.fromstring(text)
    except ET.ParseError as exc:
        raise ValueError(f"JMeter .jmx 解析失败：{exc}") from exc

    paths: Dict[str, Any] = {}
    for sampler in root.iter("HTTPSamplerProxy"):
        props = _jmx_string_props(sampler)
        path = props.get("HTTPSampler.path") or "/"
        # path 可能是完整 URL，取 path 部分，query 交由参数处理
        parsed = urlparse(path)
        clean_path = parsed.path or path or "/"
        method = (props.get("HTTPSampler.method") or "GET").lower()
        if method not in _HTTP_METHODS:
            continue
        op: Dict[str, Any] = {
            "summary": sampler.get("testname") or clean_path,
            "parameters": [],
            "responses": {},
        }
        raw_body = _jmx_bool_prop(sampler, "HTTPSampler.postBodyRaw")
        args = _jmx_arguments(sampler)
        if raw_body:
            body_text = next((v for n, v in args if not n), "")
            if body_text:
                op["requestBody"] = {"content": {"application/json": {"example": _maybe_json(body_text)}}}
        else:
            for name, value in args:
                if name:
                    op["parameters"].append({"name": name, "in": "query",
                                             "schema": {"type": "string"}, "example": value})
        for qk, qv in parse_qsl(parsed.query):
            op["parameters"].append({"name": qk, "in": "query", "schema": {"type": "string"}, "example": qv})
        paths.setdefault(clean_path, {})[method] = op

    if not paths:
        raise ValueError("JMeter .jmx 中未找到可导入的 HTTP 请求")
    return {"openapi": "3.0.0", "info": {"title": "JMeter 导入"}, "paths": paths}
