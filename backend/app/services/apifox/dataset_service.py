...
def _row_values(raw: Optional[str], columns: List[str]) -> Dict[str, str]:
    """行值按列定义补齐缺失列（空串）：历史行可能建列之前写入，values 缺 key。"""
    values = _loads(raw, {})
    if not isinstance(values, dict):  # 脏数据降级
        values = {}
    for col in columns:
...
def _out(db: Session, dataset: ApifoxDataset) -> DatasetOut:
    ...
    return DatasetOut(
        ...
        columns=columns,
        rows=[
            DatasetRowIn(values=_row_values(r.values, columns), enabled=r.enabled)
