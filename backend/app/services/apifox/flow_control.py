...
def loop_iterations(config: Dict[str, Any], variables: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    if mode == "count":
        # 次数支持 {{变量}} 插值（apifox 写法）；插值后再取整
        raw_count = apply_vars(str(config.get("count") if config.get("count") is not None else ""), variables)
        try:
            n = int(str(raw_count).strip() or 0)
    ...
    if mode == "list":
        ...
        # 列表来源支持两种写法：{{变量}} 插值取其值，或直接写变量名按名查
        ref = str(config.get("list_var") or "")
        raw = apply_vars(ref, variables) if VARIABLE_PATTERN.search(ref) else variables.get(ref, "")
        try:
