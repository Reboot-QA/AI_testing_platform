...
def delete_suite(db: Session, suite: ApifoxSuite, deleted_by: Optional[int] = None) -> None:
    """软删除：移入回收站（可还原）。"""
    suite.deleted_at = datetime.utcnow()
    suite.deleted_by = deleted_by
    db.commit()


def restore_suite(db: Session, suite: ApifoxSuite) -> None:
    """从回收站还原。"""
    suite.deleted_at = None
    db.commit()


def purge_suite(db: Session, suite: ApifoxSuite) -> None:
    """彻底删除：连同套件项物理删除，不可恢复。"""
    repo.delete_items(db, suite.id)
