...
def _run_case_block(
    ...
    for drive_row in engine.data_drive_rows(case, ctx.db):
        ctx.index += 1
        # 单元格 {{u}} 先按 global<env<runtime<case 解析（用例最高），再 merge；不含 drive_row 互引
        resolved_row = engine.resolve_drive_row_values(
            drive_row, global_vars, env_vars, ctx.runtime, case_vars
        )
        merged = engine.merge_vars(
            global_vars, env_vars, ctx.runtime, case_vars, resolved_row or {}
        )
        status: str
        detail: Dict[str, Any]
        if endpoint is None:
