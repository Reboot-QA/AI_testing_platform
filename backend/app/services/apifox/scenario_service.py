...
def list_scenarios(db: Session, project_id: int) -> List[ScenarioBrief]:
    return [
        ScenarioBrief(
            ...
            # else 是 if 的隐式容器行，前端会拍平成 elseChildren、列表里不显示为独立步骤，
            # 计入会让带 else 分支的场景每个多算 1 步，故排除
            step_count=sum(1 for st in repo.list_steps(db, s.id) if st.type != "else"),
...
def delete_scenario(db: Session, scenario: ApifoxScenario, deleted_by: Optional[int] = None) -> None:
    """软删除：移入回收站（可还原）。被引用的场景禁止删除，避免悬空子场景引用。"""
...
def restore_scenario(db: Session, scenario: ApifoxScenario) -> None:
    """从回收站还原。"""
    scenario.deleted_at = None
    db.commit()


def purge_scenario(db: Session, scenario: ApifoxScenario) -> None:
    """彻底删除：连同步骤物理删除，不可恢复。"""
    repo.delete_steps(db, scenario.id)
