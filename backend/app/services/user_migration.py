...
def migrate_user_login_lock(db: Session) -> None:
    ...
    with engine.begin() as conn:
        if "login_failed_count" not in user_columns:
            if dialect == "mysql":
                conn.execute(
                    text("ALTER TABLE users ADD COLUMN login_failed_count INT NOT NULL DEFAULT 0")
                )
            else:
                conn.execute(
                    text("ALTER TABLE users ADD COLUMN login_failed_count INTEGER NOT NULL DEFAULT 0")
                )
        if "login_locked" not in user_columns:
            if dialect == "mysql":
                conn.execute(text("ALTER TABLE users ADD COLUMN login_locked TINYINT(1) NOT NULL DEFAULT 0"))
            else:
                conn.execute(text("ALTER TABLE users ADD COLUMN login_locked BOOLEAN NOT NULL DEFAULT 0"))
    db.expire_all()
