...
TestCaseImportMode = Literal["append", "replace"]
...
def _sort_testcases(cases: List[TestCase]) -> List[TestCase]:
    return sort_testcases_for_display(cases)


def next_testcase_sort_order(db: Session, project_id: int) -> int:
    if not testcase_has_sort_order_column():
        current = db.query(func.max(TestCase.id)).filter(TestCase.project_id == project_id).scalar()
        return int(current or 0) + 1
    current = db.query(func.max(TestCase.sort_order)).filter(TestCase.project_id == project_id).scalar()
    return int(current or 0) + 1
...
def export_testcases_excel(project: Project, cases: List[TestCase]) -> Tuple[BytesIO, str]:
    ...
    for seq, case in enumerate(cases, start=1):
        ...
        display_seq = testcase_sort_order_value(case) or seq
        ws.cell(row_num, 1, display_seq)
...
def clear_project_testcases(db: Session, project_id: int) -> int:
    cases = db.query(TestCase).filter(TestCase.project_id == project_id).all()
    if not cases:
        return 0
    deleted = delete_testcases_with_execution_cleanup(db, cases)
    db.flush()
    return deleted
...
def import_testcases_from_rows(
    ...
    current_user: User,
    mode: TestCaseImportMode = "append",
) -> Tuple[int, int, int]:
    created = 0
    skipped = 0
    cleared = 0
    deduped_rows = dedupe_import_rows(rows)
    valid_rows: List[Tuple[str, Dict[str, str], Dict[str, str]]] = []
    ...
    for row in deduped_rows:
        ...
        try:
            payload = {
                ...
                "review_status": normalize_review_status(row.get("评审状态")),
            }
        except ValueError:
            skipped += 1
            continue
        valid_rows.append((title, payload, row))

    if not valid_rows:
        return created, skipped, cleared

    if mode == "replace":
        cleared = clear_project_testcases(db, project.id)
        requirement_map = _build_requirement_map(db, project.id)

    next_sort_order = next_testcase_sort_order(db, project.id)

    for index, (title, payload, row) in enumerate(valid_rows, start=1):
        ...
        if testcase_has_sort_order_column():
            ...
            case_payload["sort_order"] = sort_order
