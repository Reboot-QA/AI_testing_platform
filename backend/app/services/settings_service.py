...
AI_GEN_CONCURRENCY_KEY = "apifox_ai_gen_concurrency"
_AI_GEN_CONCURRENCY_MIN = 1
_AI_GEN_CONCURRENCY_MAX = 50
...
def _clamp_concurrency(value: int) -> int:
    return max(_AI_GEN_CONCURRENCY_MIN, min(_AI_GEN_CONCURRENCY_MAX, value))


def get_ai_gen_concurrency(db: Session) -> int:
    """apifox AI 生成并发数：优先全局设置，缺省回退 env，钳制到 [1,50]。"""
    raw = _get_setting(db, AI_GEN_CONCURRENCY_KEY)
    if raw is None:
        return _clamp_concurrency(env_settings.apifox_ai_gen_concurrency)
    try:
...
def update_ai_gen_concurrency(db: Session, value: int) -> dict:
    _set_setting(db, AI_GEN_CONCURRENCY_KEY, str(_clamp_concurrency(value)))
    db.commit()
