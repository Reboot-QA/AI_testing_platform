...
RequirementImportMode = Literal["append", "replace"]
...
EXCEL_HEADERS = ["序号", "标题", "类型", "优先级", "状态", "来源", "描述"]
HEADER_ALIASES = {
    "id": "ID",
    "序号": "序号",
    "标题": "标题",
...
def export_requirements_excel(project: Project, requirements: List[Requirement]) -> Tuple[BytesIO, str]:
    ...
    for seq, req in enumerate(requirements, start=1):
        display_seq = requirement_sort_order_value(req) or seq
        ws.append(
            [
                display_seq,
                req.title,
...
def parse_import_sort_order(row: Dict[str, str], fallback: int) -> int:
    raw = (row.get("序号") or row.get("ID") or "").strip()
    if not raw:
        return fallback
    try:
        value = int(float(raw))
...
def next_requirement_sort_order(db: Session, project_id: int) -> int:
    if not requirement_has_sort_order_column():
        current = db.query(func.max(Requirement.id)).filter(Requirement.project_id == project_id).scalar()
        return int(current or 0) + 1
    current = db.query(func.max(Requirement.sort_order)).filter(Requirement.project_id == project_id).scalar()
    return int(current or 0) + 1
...
def summarize_import_rows(rows: List[Dict[str, str]]) -> Tuple[int, int, int, List[str]]:
    total_rows = sum(1 for row in rows if row.get("标题", "").strip())
    return total_rows, total_rows, 0, []
...
def list_project_requirements(db: Session, project_id: int) -> List[Requirement]:
    ...
    return apply_requirement_list_order(query).all()
