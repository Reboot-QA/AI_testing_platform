...
class FunctionalOverviewOut(BaseModel):
    case_total: int
    pending_review_count: int
    ai_generated_count: int
...
class RequirementsOverviewOut(BaseModel):
    req_total: int
...
class LLMSettingsOut(BaseModel):
    ...
    # apifox AI 生成同任务内 LLM 并发扇出上限
    ai_gen_concurrency: int = 5
...
class AiGenConcurrencyUpdate(BaseModel):
...
class ManualTestRunPageOut(BaseModel):
    items: List[ManualTestRunSummaryOut]
    total: int
    page: int
    page_size: int
