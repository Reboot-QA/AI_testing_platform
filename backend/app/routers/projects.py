...
def _next_owner_seq(db: Session, owner_id: int) -> int:
    current = db.query(func.max(Project.owner_seq)).filter(Project.owner_id == owner_id).scalar()
    return int(current or 0) + 1


def _ensure_project_name_available(db: Session, name: str, exclude_id: Optional[int] = None) -> None:
...
def _count_requirements(db: Session, project_id: int) -> int:
    return int(
...
def _project_out(project: Project, db: Session) -> ProjectOut:
    req_count = _count_requirements(db, project.id)
...
def create_project(
    ...
    _ensure_project_name_available(db, data.name)
    project = Project(
        name=data.name,
        description=data.description,
        owner_id=current_user.id,
        owner_seq=_next_owner_seq(db, current_user.id),
...
def dashboard_stats(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    req_count = int(
        db.query(func.count(Requirement.id)).filter(Requirement.project_id.in_(project_ids)).scalar() or 0
    )
...
@router.get("/{project_id}/overview/functional", response_model=FunctionalOverviewOut)
def functional_overview(
    ...
    return get_functional_overview(db, project_id)


@router.get("/{project_id}/overview/requirements", response_model=RequirementsOverviewOut)
...
def update_project(
    ...
    if new_name is not None and new_name != project.name:
        _ensure_project_name_available(db, new_name, exclude_id=project.id)
