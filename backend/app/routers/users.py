...
from app.services.login_lock_service import admin_lock_user, admin_unlock_user
...
def lock_user_login(
    ...
    if user.id == current_admin.id:
        raise HTTPException(status_code=400, detail="不能锁定当前登录账号")
    admin_lock_user(user)
    db.commit()
