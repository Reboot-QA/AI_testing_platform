...
class DbExtractRow(BaseModel):
    """数据库操作处理器的提取项：从结果首行按列名取值写入变量。"""
...
class ProcessorRow(BaseModel):
    ...
    kind: str  # script | script_inline | database | database_script | wait | assertion | extract | contract
    enabled: bool = True
    # script（引用脚本库）
    script_id: Optional[int] = None
    script_name: str = ""  # 回显用
    script_lang: str = ""  # javascript | python（内联脚本也用此字段承载语言）
    # script_inline（内联脚本：正文直接存步骤，运行时不查脚本库）
    content: str = ""
    # database（数据库操作：环境级连接 + SQL + 首行提取；对齐场景 type=db 步骤）
    connection_id: Optional[int] = None
    sql: str = ""
    db_extracts: List[DbExtractRow] = Field(default_factory=list)
    # database_script（引用 SQL 脚本库：connection_id + sql_script_id + db_extracts，不用 sql）
    sql_script_id: Optional[int] = None
    # wait
...
# ---------- folder ----------
class DefaultHeaderOut(BaseModel):
    """自动生成的默认请求头（前端 Headers 页「自动生成」区展示，默认隐藏可展开）。"""

    name: str
    value: str
    computed: bool  # True=发送时自动计算（Host/Content-Length…），False=固定值注入
    description: str
...
class EndpointBrief(BaseModel):
    ...
    path: str
    sort_order: int
    cases_stale: bool = False  # Swagger 同步后契约变更、用例待复核（树上打标记）
