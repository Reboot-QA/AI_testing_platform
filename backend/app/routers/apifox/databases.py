...
from app.routers.apifox.database_schemas import (
    DatabaseCreate,
    DatabaseOut,
    DatabaseTestResult,
    DatabaseUpdate,
...
@router.post("/env-databases/{cid}/test", response_model=DatabaseTestResult)
...
@router.post("/environments/{eid}/databases/test-connection", response_model=DatabaseTestResult)
def test_database_config(
    eid: int, data: DatabaseCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    """按弹窗表单值探测连通性（新建/编辑保存前用）：构造临时连接（不落库）执行 SELECT 1。"""
    _env_checked(db, eid, user)
    conn = ApifoxEnvironmentDatabase(
        environment_id=eid, name=data.name, db_type=data.db_type, host=data.host,
        port=data.port, username=data.username, password=data.password, database=data.database,
    )
    result = db_executor.test_connection(conn)
    return DatabaseTestResult(passed=result["passed"], message=result.get("error") or "连接成功")
