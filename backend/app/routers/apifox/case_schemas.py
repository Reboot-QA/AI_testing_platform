...
class CaseBatchDeleteRequest(BaseModel):
    ...
    detach_refs: bool = False  # True：先从场景/套件中移除引用再删用例


class CaseBatchDeleteBlockedItem(BaseModel):
    name: str
    scenarios: List[str] = Field(default_factory=list)
    suites: List[str] = Field(default_factory=list)


class CaseBatchDeleteResult(BaseModel):
    deleted: int
    blocked: List[str] = Field(default_factory=list)  # 被引用而未删的用例名
    blocked_details: List[CaseBatchDeleteBlockedItem] = Field(default_factory=list)
...
class CaseBrief(BaseModel):
    ...
    name: str
    category: str = "other"
    origin: str = "manual"  # manual手工 | ai AI生成
    sort_order: int
    last_result: Optional[str] = None  # 最近一次运行状态：passed/failed/running（无运行为 None）
    last_error: Optional[str] = None  # 最近运行失败时的失败原因（首个失败步骤 error_message）
    last_run_at: Optional[datetime] = None  # 最近一次运行完成/开始时间
