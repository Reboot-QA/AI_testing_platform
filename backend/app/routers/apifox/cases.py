...
@router.post("/endpoints/{eid}/cases/batch-delete", response_model=CaseBatchDeleteResult)
def batch_delete_cases(
    eid: int,
    data: CaseBatchDeleteRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
):
    """批量软删除该接口下的用例（移入回收站）；被场景步骤引用的跳过并回其名。"""
    endpoint = _endpoint_checked(db, eid, user)
    deleted, blocked, blocked_details = service.batch_delete_cases(
        db, endpoint.id, data.case_ids, user.id, detach_refs=data.detach_refs
    )
    return CaseBatchDeleteResult(deleted=deleted, blocked=blocked, blocked_details=blocked_details)
