...
from app.services.test_execution_service import (
    create_manual_run,
    get_executor_name,
    sort_run_cases_by_library_order,
    submit_case_result,
)
from app.services.testcase_query_helper import apply_testcase_list_order, testcase_sort_order_value
...
def _case_out(db: Session, item: ManualTestRunCase) -> ManualTestRunCaseOut:
    ...
    return ManualTestRunCaseOut(
        ...
        testcase_sort_order=testcase_sort_order_value(case) if case else 0,
        result=item.result,
...
def _detail_out(db: Session, run: ManualTestRun) -> ManualTestRunDetailOut:
    cases = (
        ...
        .options(joinedload(ManualTestRunCase.testcase))
        .filter(ManualTestRunCase.run_id == run.id)
        .all()
    )
    cases = sort_run_cases_by_library_order(cases)
...
@router.get("/page", response_model=ManualTestRunPageOut)
def list_runs_page(
    project_id: int,
    ...
    return ManualTestRunPageOut(
        items=[_summary_out(db, run) for run in runs],
        total=total,
        page=page,
        page_size=page_size,
    )
...
def list_available_cases(
    ...
    cases = apply_testcase_list_order(query).all()
    return [
        {
            "id": case.id,
            "title": case.title,
