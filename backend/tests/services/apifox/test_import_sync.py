...
# ---------- cases_stale：同步契约变更 & 有用例 → 打「待复核」标记；增改用例清除 ----------
def test_sync_marks_cases_stale_when_changed_endpoint_has_cases(db):
    ep = _endpoint(db, "GET", "/x")  # 无 query
    _case(db, ep)  # 有用例
    doc = _build_doc([{"method": "GET", "path": "/x", "query": ("q",)}])  # 契约变更(加 query)

    sync.apply_sync(db, PID, doc, False)

    db.refresh(ep)
    assert ep.cases_stale is True


def test_sync_no_stale_when_no_cases(db):
    ep = _endpoint(db, "GET", "/x")  # 无用例
    doc = _build_doc([{"method": "GET", "path": "/x", "query": ("q",)}])
...
def test_create_case_clears_stale(db):
    from app.routers.apifox.case_schemas import CaseCreate
    from app.services.apifox import case_service

    ep = _endpoint(db, "GET", "/x")
    ep.cases_stale = True
    db.commit()

    case_service.create_case(db, PID, ep.id, CaseCreate(name="新用例"))
