...
def test_send_request_unicode_encode_error_returns_failed_detail(db, monkeypatch):
    def _raise_unicode(*args, **kwargs):
        raise UnicodeEncodeError("ascii", "中文", 0, 2, "ordinal not in range(128)")

    monkeypatch.setattr(httpx.Client, "request", _raise_unicode)
    detail = {}

    resp = run_engine._send_request(_plan(), detail)

    assert resp is None
    assert detail["error_message"] and "请求编码失败" in detail["error_message"]
