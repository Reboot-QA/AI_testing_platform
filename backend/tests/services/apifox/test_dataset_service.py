...
def test_row_missing_column_backfilled_on_read(db):
    # 历史行可能在建列之前写入，values 缺 key（线上 id=4 即 columns=[] 时存了 2 行）。
    # 读取时按列定义补空串，否则执行迭代不绑定该变量、{{列名}} 会原样发到目标接口。
    out = _create(db, columns=[], rows=[DatasetRowIn(values={})])
    ds = _get(db, out.id)

    updated = svc.update_dataset(db, ds, DatasetUpdate(columns=["username", "password"]))

    assert updated.rows[0].values == {"username": "", "password": ""}


def test_row_keeps_existing_values_when_backfilling(db):
    out = _create(db, columns=["a"], rows=[DatasetRowIn(values={"a": "x"})])
    ds = _get(db, out.id)

    updated = svc.update_dataset(db, ds, DatasetUpdate(columns=["a", "b"]))

    assert updated.rows[0].values == {"a": "x", "b": ""}
