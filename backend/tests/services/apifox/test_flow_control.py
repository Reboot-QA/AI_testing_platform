...
# ---------- #5：循环次数/列表支持 {{变量}} 插值 ----------
def test_count_mode_interpolates_variable():
    its = loop_iterations({"mode": "count", "count": "{{n}}"}, {"n": "3"})

    assert [x["index"] for x in its] == ["0", "1", "2"]


def test_list_mode_interpolates_variable_reference():
    its = loop_iterations(
        {"mode": "list", "list_var": "{{items}}", "item_var": "it"}, {"items": '["a", "b"]'}
    )
...
def test_list_mode_bare_name_still_supported():
    its = loop_iterations(
        {"mode": "list", "list_var": "items", "item_var": "it"}, {"items": '["x"]'}
    )

    assert its == [{"it": "x", "index": "0"}]
