...
def test_delete_suite_soft_removes_from_list_keeps_items(db, make_case):
    a = make_case(name="A")
    out = svc.create_suite(db, project_id=1, data=SuiteCreate(name="x", items=[_case_item(a.id)]))
    suite = svc.repo.get_suite(db, out.id)
    ...
    # 软删：从主列表消失，但行仍在（回收站可还原），套件项保留
    assert svc.list_suites(db, project_id=1) == []
    assert svc.repo.get_suite(db, out.id).deleted_at is not None
    assert len(svc.repo.list_items(db, out.id)) == 1


def test_purge_suite_removes_items(db, make_case):
    a = make_case(name="A")
    out = svc.create_suite(db, project_id=1, data=SuiteCreate(name="x", items=[_case_item(a.id)]))
    suite = svc.repo.get_suite(db, out.id)
    svc.delete_suite(db, suite)

    svc.purge_suite(db, suite)
