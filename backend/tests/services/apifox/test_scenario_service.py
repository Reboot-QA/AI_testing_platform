...
def test_step_count_excludes_else_container(db, make_case):
    # else 是 if 的隐式容器行，前端拍平成 elseChildren、列表不显示为独立步骤；
    # 计入会让带 else 分支的场景每个多算 1 步（TC-F-110 曾显示 13 步、实际可见 12 步）
    case = make_case()
    svc.create_scenario(
        db,
        project_id=1,
        data=ScenarioCreate(
            ...
            steps=[
                _case_step(case.id),
                _if([_case_step(case.id)], else_children=[_case_step(case.id)]),
            ],
        ),
    )

    brief = next(s for s in svc.list_scenarios(db, project_id=1) if s.name == "带 else 的场景")
    # 可见步骤：case + if + then 内 case + else 内 case = 4（不含 else 容器本身）
    assert brief.step_count == 4


def test_step_count_without_else_unchanged(db, make_case):
    case = make_case()
    svc.create_scenario(
        db,
        project_id=1,
        data=ScenarioCreate(
            name="无 else 的场景",
            steps=[_case_step(case.id), _if([_case_step(case.id)])],
        ),
    )

    brief = next(s for s in svc.list_scenarios(db, project_id=1) if s.name == "无 else 的场景")
