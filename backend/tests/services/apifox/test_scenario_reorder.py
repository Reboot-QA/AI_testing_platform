...
乐观锁版本冲突与递增。方案 A 快照重编号：只发受影响的组，后端只更新传入 id。
"""
...
def _project(db, project_id=1, order_version=1):
    p = Project(
        id=project_id, name=f"排序项目{project_id}", owner_id=1, scenario_order_version=order_version
    )
    db.add(p)
    db.commit()
    return p
...
def test_reorder_within_group_updates_sort_order(db):
    ...
    # 拖成 c, a, b
    service.reorder_scenarios(db, 1, [_item(c.id, None, 0), _item(a.id, None, 1), _item(b.id, None, 2)], 1)
...
def test_reorder_moves_scenario_into_folder(db):
    ...
    service.reorder_scenarios(db, 1, [_item(s.id, folder.id, 0)], 1)
...
def test_reorder_moves_scenario_to_ungrouped(db):
    ...
    service.reorder_scenarios(db, 1, [_item(s.id, None, 0)], 1)
...
def test_reorder_only_touches_passed_ids(db):
    ...
    service.reorder_scenarios(db, 1, [_item(a.id, None, 9)], 1)
...
def test_reorder_rejects_unknown_scenario(db):
    ...
    with pytest.raises(ValueError):
        service.reorder_scenarios(db, 1, [_item(99999, None, 0)], 1)
...
def test_reorder_rejects_scenario_from_other_project(db):
    ...
    with pytest.raises(ValueError):
        service.reorder_scenarios(db, 1, [_item(other.id, None, 0)], 1)
...
def test_reorder_rejects_folder_from_other_project(db):
    ...
    with pytest.raises(ValueError):
        service.reorder_scenarios(db, 1, [_item(s.id, foreign_folder.id, 0)], 1)


# ---------- 乐观锁 ----------
def test_reorder_rejects_stale_order_version(db):
    _project(db, order_version=3)
    s = _scenario(db, name="s")

    # 传入的期望版本(1) 落后于当前(3)：冲突，不落库
    with pytest.raises(service.OrderVersionConflictError):
        service.reorder_scenarios(db, 1, [_item(s.id, None, 5)], 1)

    db.refresh(s)
    assert s.sort_order == 0  # 冲突后未写入


def test_reorder_bumps_order_version(db):
    _project(db, order_version=1)
    s = _scenario(db, name="s")

    new_version, count = service.reorder_scenarios(db, 1, [_item(s.id, None, 1)], 1)

    assert (new_version, count) == (2, 1)  # 成功后排序版本 +1
    assert db.get(Project, 1).scenario_order_version == 2
...
def test_reorder_does_not_bump_version(db):
    ...
    service.reorder_scenarios(db, 1, [_item(s.id, None, 1)], 1)

    db.refresh(s)
    assert s.version == v0  # 排序不算内容编辑，不动场景乐观锁版本
