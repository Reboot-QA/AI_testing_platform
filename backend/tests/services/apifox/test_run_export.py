...
def test_report_time_is_human_readable_not_timestamp():
    """报告内容里的"报告时间"应是可读格式，而非紧凑时间戳串（Confluence 7/24-#21）。"""
    from app.services.apifox.run_export_common import _report_summary_rows

    rows = dict(_report_summary_rows(_report()))

    assert rows["报告时间"] == "2026-07-15 10:00:00"  # 修复前为 "20260715100000"


def test_export_filename_still_uses_compact_timestamp():
    """回归：下载文件名仍用紧凑时间（含无分隔符时间戳），避免文件名带非法字符。"""
    name = ex.build_export_filename([_report()], "xlsx")

    assert "20260715100000" in name
