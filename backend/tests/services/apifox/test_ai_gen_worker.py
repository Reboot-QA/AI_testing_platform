...
def test_run_gen_with_retry_succeeds_on_second_attempt(monkeypatch):
    import asyncio
    ...
    async def flaky(plan, cfg, on_batch):
        calls["n"] += 1
        if calls["n"] == 1:
            raise ValueError("LLM 请求超时")
        return [CaseCreate(name="c", category="positive", request_spec=RequestSpec())]

    monkeypatch.setattr(worker.ai_case_service, "run_gen_plan_incremental", flaky)

    cases = asyncio.run(worker._run_gen_with_retry(None, {}, 1, lambda _b: None))

    assert calls["n"] == 2 and cases[0].name == "c"  # 第一次超时，第二次成功


def test_run_gen_with_retry_raises_after_max_attempts(monkeypatch):
    import asyncio

    async def always_fail(plan, cfg, on_batch):
        raise ValueError("boom")

    monkeypatch.setattr(worker.ai_case_service, "run_gen_plan_incremental", always_fail)

    with pytest.raises(ValueError):
        asyncio.run(worker._run_gen_with_retry(None, {}, 1, lambda _b: None))


def test_ai_gen_concurrency_setting_overrides_env_and_clamps(db, monkeypatch):
    from app.services import settings_service as ss

    monkeypatch.setattr(ss.env_settings, "apifox_ai_gen_concurrency", 7)
    assert ss.get_ai_gen_concurrency(db) == 7  # 无设置 → 回退 env

    ss._set_setting(db, ss.AI_GEN_CONCURRENCY_KEY, "12")
    db.commit()
    assert ss.get_ai_gen_concurrency(db) == 12  # 全局设置优先于 env

    ss._set_setting(db, ss.AI_GEN_CONCURRENCY_KEY, "999")
    db.commit()
    assert ss.get_ai_gen_concurrency(db) == 50  # 上限钳制

    ss._set_setting(db, ss.AI_GEN_CONCURRENCY_KEY, "0")
    db.commit()
    assert ss.get_ai_gen_concurrency(db) == 1  # 下限钳制，避免 Semaphore(0) 卡死
