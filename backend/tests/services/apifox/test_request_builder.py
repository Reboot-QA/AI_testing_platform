...
# ---------- 自动生成的默认头 ----------
def test_fixed_default_headers_injected_when_absent():
    plan = _build(_endpoint(path="/x"), environment=_env(base_url="https://api.test"))

    h = plan["headers"]
    assert h["User-Agent"] == "DB-testlab/1.0.0"
    assert h["Accept"] == "*/*"
    assert h["Accept-Encoding"] == "gzip, deflate"
    assert h["Connection"] == "keep-alive"


def test_user_header_overrides_default_case_insensitive():
    spec = {"headers": [{"key": "user-agent", "value": "MyClient/9"}]}
    plan = _build(_endpoint(path="/x"), spec=spec, environment=_env(base_url="https://api.test"))

    # 用户配了（即使大小写不同）就不注入默认，保留用户值
    ua = [v for k, v in plan["headers"].items() if k.lower() == "user-agent"]
    assert ua == ["MyClient/9"]


def test_computed_headers_not_statically_injected():
    plan = _build(_endpoint(path="/x"), environment=_env(base_url="https://api.test"))

    lower = {k.lower() for k in plan["headers"]}
    # Host/Content-Length 由发送时(httpx)生成，不静态注入
    assert "host" not in lower
    assert "content-length" not in lower


def test_apply_default_headers_unit_is_idempotent():
    from app.services.apifox.default_headers import apply_default_headers

    headers = {"X-Custom": "1"}
    apply_default_headers(headers)
    apply_default_headers(headers)  # 二次调用不重复/不报错
...
# ---------- 非 ASCII URL / 请求头 ----------
def test_chinese_path_percent_encoded_in_url():
    plan = _build(
        _endpoint(path="/用户/登录"),
        environment=_env(base_url="https://api.test"),
    )

    assert plan["url"] == "https://api.test/%E7%94%A8%E6%88%B7/%E7%99%BB%E5%BD%95"


def test_already_encoded_path_segments_preserved():
    plan = _build(
        _endpoint(path="/users/%E4%B8%AD"),
        environment=_env(base_url="https://api.test"),
    )

    assert plan["url"] == "https://api.test/users/%E4%B8%AD"


def test_chinese_header_value_raises_clear_value_error():
    spec = {"headers": [{"key": "X-Note", "value": "中文备注"}]}
    with pytest.raises(ValueError, match="请求头「X-Note」"):
        _build(_endpoint(path="/s"), spec=spec, environment=_env(base_url="https://api.test"))


def test_chinese_cookie_in_cookie_header_raises():
    spec = {"cookies": [{"key": "note", "value": "中文"}]}
    with pytest.raises(ValueError, match="请求头「Cookie」"):
        _build(_endpoint(path="/s"), spec=spec, environment=_env(base_url="https://api.test"))
