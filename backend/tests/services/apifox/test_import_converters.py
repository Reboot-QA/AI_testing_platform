                    ...
                    "items": [
                        {
                            ...
                            "api": {
                                ...
                                "name": "查订单",
                                "parameters": {
                                    "query": [{"name": "status", "required": True, "type": "string"}],
                                    "header": [{"name": "X-Token", "type": "string", "example": "t"}],
                                },
                            },
                        },
                        {
...
def test_apifox_converts_and_parses():
    doc = conv.to_openapi3(json.dumps(APIFOX))

    eps = {(e["method"], e["path"]): e for e in import_service.parse_openapi(doc)}
    assert set(eps) == {("GET", "/orders"), ("POST", "/orders")}
    get = eps[("GET", "/orders")]
    assert any(q.key == "status" for q in get["request_spec"].query)
    assert any(h.key == "X-Token" for h in get["request_spec"].headers)
    assert get["folder"] == "订单分组"  # 末级目录名 → tag
    assert eps[("POST", "/orders")]["request_spec"].body.type == "json"


def test_apifox_empty_raises():
    with pytest.raises(ValueError):
        conv.to_openapi3(json.dumps({"apifoxProject": "1.0.0", "apiCollection": []}))
...
<jmeterTestPlan version="1.2">
  <hashTree>
    <HTTPSamplerProxy testname="查用户">
      <stringProp name="HTTPSampler.domain">api.x</stringProp>
      ...
      <elementProp name="HTTPsampler.Arguments" elementType="Arguments">
        <collectionProp name="Arguments.arguments">
          <elementProp name="" elementType="HTTPArgument">
            <stringProp name="Argument.value">{"n":1}</stringProp>
          </elementProp>
        </collectionProp>
      </elementProp>
    </HTTPSamplerProxy>
...
def test_jmeter_converts_and_parses():
    doc = conv.to_openapi3(JMX)

    eps = {(e["method"], e["path"]): e for e in import_service.parse_openapi(doc)}
    assert set(eps) == {("GET", "/users"), ("POST", "/users")}
    assert any(q.key == "page" for q in eps[("GET", "/users")]["request_spec"].query)
    assert eps[("POST", "/users")]["request_spec"].body.type == "json"


def test_jmeter_empty_raises():
    with pytest.raises(ValueError):
        conv.to_openapi3('<?xml version="1.0"?><jmeterTestPlan><hashTree></hashTree></jmeterTestPlan>')
