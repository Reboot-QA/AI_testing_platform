...
# ---------- 契约：数据驱动单元格预插值 resolve_drive_row_values ----------
def test_resolve_drive_row_case_vars_win_over_env_and_global():
    drive = {"username": "{{u}}", "password": "{{p}}"}
    resolved = resolve_drive_row_values(
        drive,
        {"u": "g", "p": "g"},
        {"u": "e", "p": "e"},
        {},
    ...
    assert resolved == {"username": "c", "password": "c"}


def test_resolve_drive_row_env_wins_over_global_without_case():
    drive = {"username": "{{u}}"}
    resolved = resolve_drive_row_values(drive, {"u": "g"}, {"u": "e"}, {}, {})
    assert resolved == {"username": "e"}


def test_resolve_drive_row_missing_keeps_placeholder():
    drive = {"username": "{{u}}"}
    resolved = resolve_drive_row_values(drive, {}, {}, {}, {})
    assert resolved == {"username": "{{u}}"}


def test_resolve_drive_row_none_passthrough():
    assert resolve_drive_row_values(None, {"u": "x"}) is None


def test_contract_body_after_drive_cell_preresolve():
    """列 username 单元格 {{u}} + lookup 有 u → Body {{username}} 发出真实值。"""
    drive_raw = {"username": "{{u}}", "password": "{{p}}"}
    body = '{"username":"{{username}}","password":"{{password}}"}'
    resolved = resolve_drive_row_values(drive_raw, {"u": "tyla001", "p": "tyla001"}, {}, {}, {})
    merged = merge_vars({}, {}, {}, {}, resolved or {})
    assert apply_vars(body, merged) == '{"username":"tyla001","password":"tyla001"}'
