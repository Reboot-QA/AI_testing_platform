...
def test_apply_kv_null_disables_existing_param():
    """AI 用 null 表达「本用例不传该参数」（必选参数缺失负向用例）→ 该行禁用，不发送。"""
    rows = [KvRow(key="username", value="alice", enabled=True)]

    svc._apply_kv(rows, {"username": None})

    assert rows[0].enabled is False


def test_apply_kv_null_skips_absent_key():
    """null 值但参数不在基础 spec 里 → 不新增一行 null 参数。"""
    rows = [KvRow(key="page", value="1", enabled=True)]

    svc._apply_kv(rows, {"missing": None})

    assert [r.key for r in rows] == ["page"]
...
def test_build_case_missing_param_disables_form_field():
    """「缺少用户名登录」：base 表单含 username(enabled)，AI 传 username=null → 用例里 username 禁用不发送。"""
    base = _spec(body=BodySpec(
        type="form-data",
        form=[KvRow(key="username", value="a", enabled=True), KvRow(key="password", value="p", enabled=True)],
    ))

    case = svc._build_case(
        base,
        {"name": "缺少用户名登录", "category": "negative", "body": {"username": None, "password": "p"}},
    )

    assert case is not None
    by_key = {r.key: r for r in case.request_spec.body.form}
    assert by_key["username"].enabled is False  # 不传用户名
    assert by_key["password"].enabled is True


def test_build_case_missing_query_param_disabled():
    base = _spec(query=[KvRow(key="token", value="t", enabled=True)])

    case = svc._build_case(base, {"name": "缺少 token", "category": "negative", "query": {"token": None}})

    assert case is not None and case.request_spec.query[0].enabled is False


def test_build_case_json_body_drops_null_field():
    """JSON body：AI 把缺失字段设 null → 序列化时剔除该键（等价不传）。"""
    import json as _json

    base = _spec(body=BodySpec(type="json"))

    case = svc._build_case(base, {"name": "缺少用户名", "body": {"username": None, "password": "p"}})

    assert case is not None
    assert _json.loads(case.request_spec.body.raw) == {"password": "p"}


def test_build_user_prompt_instructs_null_for_missing_param():
    prompt = svc.build_user_prompt("接口：POST /login\n", [AiGenCategory(category="negative", count=2)])

    assert "null" in prompt  # 指导 AI 用 null 表达「本用例不传该参数」
...
# ---------- 已有用例：逐类别覆盖感知提示 ----------
def test_build_user_prompt_no_coverage_note_when_no_existing():
    prompt = svc.build_user_prompt("接口：GET /x\n", [AiGenCategory(category="boundary", count=1)])

    assert "该类别已有" not in prompt


def test_build_user_prompt_annotates_only_covered_category():
    cats = [AiGenCategory(category="negative", count=2), AiGenCategory(category="boundary", count=2)]
    # 已有逆向用例，无边界值用例
    existing = [("缺少必填参数 page 返回 400", "negative")]

    prompt = svc.build_user_prompt("接口：GET /x\n", cats, existing)

    # 逆向类别下给出已有用例 + 去重提示；边界值类别不带该提示
    neg_part, bnd_part = prompt.split("boundary（")
    assert "该类别已有 1 条用例" in neg_part
    assert "缺少必填参数 page 返回 400" in neg_part
    assert "测试方向不同" in neg_part
    assert "该类别已有" not in bnd_part


def test_build_user_prompt_ignores_existing_of_unrequested_category():
    cats = [AiGenCategory(category="boundary", count=2)]
    # 只请求边界值，已有的正向用例不应影响边界值提示
    existing = [("正常查询返回 200", "positive")]
