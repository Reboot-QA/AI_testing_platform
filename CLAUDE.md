...
- **规则单一事实来源在 `.claude/rules/`**：`frontend.md`、`backend.md`、`git-workflow.md`、`spec-workflow.md`。改规则改这四份；`frontend/CLAUDE.md` / `backend/CLAUDE.md` 与 `.cursor/rules/*.mdc` 都是引用它们的薄封装。
- **Spec 驱动 + Issue/MR 半自主工作流**：见 `.claude/rules/spec-workflow.md`（配套技能 `.claude/skills/{grill-with-docs,to-spec,implement-and-ship,handoff}`）。对齐 → 出规格发 Issue → 实施建 MR，纯 `glab`、合 `dev`、不自动关 Issue。**Spec 正文的单一事实来源是 GitLab Issue**，仓库内不存 Spec 文件；跨端/跨会话接力的交接物是 Issue 评论（`handoff` 技能）。中文 Issue 正文**必须**走 `.claude/scripts/glab_cn.py`（本机 GBK 控制台 + glab 无 `--description-file`，拼参数会写坏正文）。
...
- 后端硬约束详见 `.claude/rules/backend.md`（→ `backend/docs/后端开发规范.md`）。
